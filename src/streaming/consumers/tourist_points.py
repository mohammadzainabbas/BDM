...
def get_touristic_points_data_schema() -> StructType:
    """
    Return the data schema for touristic_points
    """
    return StructType([
...
def get_api_touristic_points_data_schema() -> StructType:
    """
    Return the data schema for touristic_points coming from the API calls
...
def get_touristic_points_from_stream() -> None:
    r"""
    ...
    # __hdfs_location = "{}/{}".format(hdfs_home, join("formatted_data", "touristic_points"))
    __hdfs_location = "{}/{}".format(hdfs_home, join("formatted_data", "touristic_points"))
    ...
    __schema = get_api_touristic_points_data_schema()
    ...
    __df = __df.select(SF.from_json(SF.col("formatted_value"), __schema).alias("touristic_points_records"), "timestamp")
    __df = __df.select("touristic_points_records.*", "timestamp")
    
    # required columns
    __columns = required_columns()

    # update the schema (we can't do this in streaming sources)
    # __new_schema = get_touristic_points_data_schema()
...
def main() -> None:
    ...
    get_touristic_points_from_stream()
